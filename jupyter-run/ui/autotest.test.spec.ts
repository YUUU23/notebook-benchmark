...
test.describe.serial("Notebook Run", () => {
  ...
  test("Run notebook initially and capture cell outputs", async ({
    ...
    // Download first run of notebook
    await page.notebook.save();
    console.log(`== SAVING ORIGINAL NOTEBOOK IN: ${downloadPath}`);
    await page.getByText("File", { exact: true }).click();
    const downloadPromiseOriginal = page.waitForEvent("download");
    await page.getByRole("menuitem", { name: "Download" }).click();
    const downloadOriginal = await downloadPromiseOriginal;
    await downloadOriginal.saveAs(downloadPath);
